...
const JournalCarousel: React.FC = () => {
  ...
  const sliderRef = useRef<Slider>(null); // Added ref for manual control
  ...
  const navigateToUrl = (id: string) => {
    const bookDetails = BookData.find((x: { id: string }) => x.id === id);
    if (bookDetails) {
      const journalTitle = bookDetails.journalTitle;
      let formattedTitle = journalTitle.split('(')[0].trim().replace(/\s+/g, '-').replace(/-$/, '');
      router.push(`/${id}/${formattedTitle}/About`);
    }
  };

  const settings = {
    dots: false,
    arrows: false, // IMPORTANT: disable default slick arrows
    infinite: true,
    speed: 500,
    slidesToShow: 3,
    slidesToScroll: 1,
